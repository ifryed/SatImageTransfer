...
def saveLap(base_path, lap_pyr, max_size, verbos=False):
    ...
    for i, lap in enumerate(lap_pyr):
        ...
        save_path = "{}_{}.png".format(base_path, i)
        save_path_jp2 = save_path[:-3] + 'jp2'

        if min(lap.shape[:2]) < 10:
            with open("{}_{}.bin".format(base_path, i, offset), 'wb') as fp:
                lap = lap.astype(np.uint8)
                h, w = lap.shape[:2]
...
def CompressImage(in_img: np.ndarray, path: str, max_size: int = 16 * KBYTE, comp_gray=True):
    if in_img is None:
